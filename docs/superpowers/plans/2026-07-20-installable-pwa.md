...
인터넷이 없어도 앱 창은 뜨지만, 습관 데이터는 서버에서만 가져옵니다. 연결이 없으면 안내 배너가
표시되고, 연결이 돌아오면 자동으로 새로고침됩니다.
...
```js
const CACHE_VERSION = "hg-v2";   // → "hg-v3"
```
...
- **Cache Storage**: `hg-v2` 캐시 안에 7개 항목(`/`, `/style.css`, `/app.js`, `/manifest.json`, 아이콘 3개)이 있어야 한다

localhost는 https 예외로 취급되므로 로컬에서도 설치 아이콘이 나타납니다.

- [ ] **Step 5: 오프라인 동작을 확인한다 (수동)**

1. 게스트 모드로 습관 하나를 추가한다
2. DevTools → Network → Throttling을 **Offline**으로 바꾼다
3. 페이지를 새로고침한다
   - 기대: 헤더·배경이 정상적으로 뜨고, 하단에 "인터넷 연결을 확인해 주세요." 배너, 본문에 "연결되면 습관을 다시 불러올게요."
   - "불러오는 중…"에서 멈추면 Task 4가 잘못된 것이다
4. Throttling을 **No throttling**으로 되돌린다
   - 기대: 자동으로 새로고침되고 습관 목록이 돌아온다
...
3. `CACHE_VERSION`을 `hg-v3`으로 올리고 재시작 후 새로고침한다
   - 기대: 배경이 분홍색으로 바뀌고, Cache Storage에서 `hg-v2`가 사라지고 `hg-v3`만 남는다
4. `--paper`를 `#F3F5EF`로, `CACHE_VERSION`을 `hg-v2`로 되돌린다
